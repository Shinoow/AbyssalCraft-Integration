	...
	public void onUpdate(ItemStack itemstack, EntityPlayer player) {
		if(player.worldObj.provider.getDimensionId() == 1){
			if(((ItemWand)itemstack.getItem()).getCap(itemstack) == WandCap.caps.get("void")){
				((ItemWand)itemstack.getItem()).getRod(itemstack).setCapacity(500);
				for(int i = 0; i < primals.length; i++)
					if(((ItemWand)itemstack.getItem()).getVis(itemstack, primals[i])< 50000)
						((ItemWand)itemstack.getItem()).addVis(itemstack, primals[i], 50000, true);
			}
		}
			...
			for(int i = 0; i < primals.length; i++)
				if(((ItemWand)itemstack.getItem()).getVis(itemstack, primals[i]) > 0)
					((ItemWand)itemstack.getItem()).consumeVis(itemstack, player, primals[i], 500, false);
